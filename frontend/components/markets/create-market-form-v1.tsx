...
import {
  ...
  FlightData,
  generateFlightId,
} from "@/hooks/useFlightFetch";
import {
  useWriteSkyOddsCreateFlightMarket,
  useReadSkyOddsAdminRole,
  useReadSkyOddsHasRole,
...
export function CreateMarketForm() {
  ...
  const { data: hasAdminRole, isLoading: isRoleLoading } =
    useReadSkyOddsHasRole({
  ...
  const { writeContractAsync, isPending: isCreating } =
    useWriteSkyOddsCreateFlightMarket();
  ...
  useEffect(() => {
    const loadFlights = async () => {
      ...
      const transformed: FlightData[] = rawFlights.map((f: any) => ({
        ...
        scheduledDeparture: Math.floor(
          new Date(f.departure.scheduled).getTime() / 1000
        ),
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      <Card className="border-zinc-200">
        ...
        <CardContent className="space-y-6">
              ...
              <div className="p-4 bg-red-50 border border-red-200 rounded-xl flex gap-3 text-red-700">
                ...
                <div className="text-sm">
                  ...
                  <p className="opacity-90">
                    Only authorized admins can initialize new markets. Your
                    wallet (`{address.slice(0, 6)}...{address.slice(-4)}`) does
                    not have the `ADMIN_ROLE`.
                  </p>
                </div>
              </div>
            )}
          ...
          {isConnected &&
            mntBalance &&
            parseFloat(formatUnits(mntBalance.value, mntBalance.decimals)) <
              0.01 && (
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl flex gap-3 text-amber-700">
                ...
                <div className="text-sm">
                  ...
                  <p className="opacity-90 mb-2">
                    You need Mantle Sepolia (MNT) tokens to pay for gas when
                    creating markets.
                  </p>
                  <a
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span
                  className={cn(
                    "font-medium",
                    !selectedFlight ? "text-zinc-400" : "text-zinc-900"
                  )}
                >
                  {isFetching
                    ? "Fetching flights..."
                  ...
                  >
                    ...
                    <div className="text-right">
                      <span className="text-xs font-mono font-bold text-zinc-400">
                        {new Date(
                          flight.scheduledDeparture * 1000
                        ).toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
            ...
            <div className="p-6 bg-zinc-50 border border-zinc-200 rounded-2xl space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-zinc-500 text-sm font-semibold uppercase tracking-widest">
                    {selectedFlight.airlineCode} Airways
                  </p>
                </div>
                <Badge
                  variant="outline"
                  className="bg-white text-zinc-600 border-zinc-200 py-1 px-3"
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2 text-zinc-900 font-bold">
                    ...
                    {new Date(
                      selectedFlight.scheduledDeparture * 1000
                    ).toLocaleString([], {
                      dateStyle: "medium",
                      timeStyle: "short",
                    })}
              ...
              <div className="space-y-2 pt-4 border-t border-zinc-200">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-[10px] font-bold text-zinc-400 bg-zinc-100 px-2 py-0.5 rounded italic">
                    Higher b = Stable Price
                  </span>
                </div>
                <Input
                  type="number"
